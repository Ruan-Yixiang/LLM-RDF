...
def get_data(f):
    """
    Extracts project data and organizes it for response.

    Parameters:
    - f: The loaded project object.

    Returns:
    - Tuple containing project information, device status, reaction status, reaction info, plate info, result chart, PI chart, and result table.
    """
    log = f.log[-1]
    project_info = {
...
async def load(data: IdData):
    ...
    Returns:
    - JSON response with project information, device status, reaction status, reaction info, plate info, result chart, PI chart, and result table.
    """
    id_data = str(data.id)
    ...
    for i in pkl_files:
        if i.split(os.path.sep)[-1].split("_")[0] == id_data:
            with open(i, "rb") as file:
                f = pickle.load(file)
                project_info, device_status, reaction_status, reaction_info, plate_info, res_chart, pi_chart, res_table = get_data(f)
...
async def reboot(data: IdData):
    """
    Resets the project based on the provided ID and creates a backup of the current state.
    ...
    pkl_files = glob.glob(os.path.join('project', "*.pkl"))
    for i in pkl_files:
        if i.split(os.path.sep)[-1].split("_")[0] == project_id:
            shutil.copyfile(i, project_id + '_' + time.strftime('%Y%m%d%H%M%S', time.localtime()) + '.pkl')
            with open(i, "rb") as file:
                bo = pickle.load(file)
