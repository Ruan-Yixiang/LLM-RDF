...
full_path = 'smiles2img/'
project_path = 'project/'


def transparent(_ori):
    """
    Converts the white background of an image to transparent.

    Parameters:
    - _ori (PIL.Image): The original image.

    Returns:
    - PIL.Image: The image with a transparent background.
    """
...
def smiles2montageimg(_data):
    """
    Converts a SMILES string to a montage image, including reactions, solvents, and products.

    Parameters:
    - _data (dict): The input data containing the SMILES string.

    Returns:
    - list: Paths to the generated montage images.
    """
    smiles = _data['smiles']
    ...
    # if solvents != ['']:
    url_path = [full_path + current_time + uu + '_equation.png', full_path + current_time + uu + '_solvents.png']
...
def nlp2space(data: NLPactionData):
    """
    Converts natural language description and action steps into a structured space JSON.

    Parameters:
    - data (NLPactionData): The input data containing natural language description and action steps.

    Returns:
    - dict: The structured space JSON and modified action steps.
    """
    nlp = data.nlp
    action = data.action
    ...
    res = '{"Space": [{"name": "Time","type": "continuous","range": [45, 90]}, {"name": "volume of Base","type": "continuous","range": [0.125, 0.25]}, {"name": "Cu catalyst","type": "categorical", "range": ["CuCl2", "CuBr2", "Cu(BF4)2", "Cu(OTf)2"]}, {"name": "Base","type": "categorical", "range": ["NMI", "DBU"]}]}'
...
async def get_image(data: SmilesData):
    """
    Converts a SMILES string to a montage image and saves it.

    Parameters:
    - data (SmilesData): The input data containing the SMILES string.
...
def nlp2procedure(data: NLPData):
    """
    Converts natural language description of an experimental procedure into a structured procedure and action steps.

    Parameters:
    - data (NLPData): The input data containing the natural language description and operation type.

    Returns:
    - dict: The structured procedure, action steps, and parameters for the unchained process.
    """
    nlp = data.nlp
    ...
    if operation == 'synthesis':
        procedure = "1. dispense 0.25 mL Substrate from mother plate cell to reactor plate cell using SyringePump,ExtSingleTip\n2. dispense 0.25 mL Cu catalyst from mother plate cell to reactor plate cell using SyringePump,ExtSingleTip\n3. dispense 0.25 mL TEMPO from mother plate cell to reactor plate cell using SyringePump,ExtSingleTip\n4. dispense 0.25 mL base from mother plate cell to reactor plate cell using SyringePump,ExtSingleTip"
...
async def create_project(data: ParaData):
    """
    Creates a new Bayesian Optimization (BO) project using the provided parameters.

    Parameters:
    - data (ParaData): The input data containing project parameters.
...
async def unchained_step(data: StepData):
    """
    Converts step data into parameters for the unchained synthesis process.

    Parameters:
    - data (StepData): The input data containing step details.

    Returns:
    - dict: The parameters for the unchained synthesis process.
    """
    step_data = data.step
