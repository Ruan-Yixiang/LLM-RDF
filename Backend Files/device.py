...
class Reaction:
    ...
    Attributes:
    - reaction_number: Unique identifier for the reaction.
    - reaction_time: Target reaction time.
    - reactor_loc: Location of the reactor.
    - step: Current step of the reaction.
    - start_time: Time when the reaction started.
    - end_time: Time when the reaction ended.
    - condition: Conditions of the reaction.
    - reaction_time_actual: Actual reaction time.
    - x: Features tensor for the reaction.
    - y: Target tensor for the reaction.
    """
    ...
    def update(self):
        """
        Updates the status of the reaction based on the elapsed time.
        """
        now = time.time()
...
class LCPlate:
    """
    A class to represent an LC plate.

    Attributes:
    - status: Current status of the plate ('idle', 'loaded').
    - reaction: The reaction currently associated with the plate.
    """
    def __init__(self):
...
class LC:
    """
    A class to represent a Liquid Chromatography (LC) instrument.

    Attributes:
    - name: Name of the LC instrument.
    - ip: IP address of the LC instrument.
    - template: Template for the LC instrument.
    - local_path: Local path to the configuration file.
    - remote_path: Remote path to the configuration file.
    - reaction: The reaction currently associated with the LC instrument.
    - status: Current status of the LC instrument.
    - waiting_analyze: List of reactions waiting to be analyzed.
    - history_analyze: List of reactions that have been analyzed.
    - now_analyze: List of reactions currently being analyzed.
    - log: Log of analysis operations.
...
class LcArm:
    """
    A class to represent an LC Arm for handling LC plates.

    Attributes:
    - name: Name of the LC Arm.
    - status: Current status of the LC Arm.
    - ip: IP address of the LC Arm.
    - up_local_path: Local path for uploading files.
    - up_remote_path: Remote path for uploading files.
    - down_local_path: Local path for downloading files.
    - down_remote_path: Remote path for downloading files.
    - waiting_upload: List of reactions waiting to be uploaded.
    - history_upload: List of reactions that have been uploaded.
    - now_upload: List of reactions currently being uploaded.
    - waiting_download: List of reactions waiting to be downloaded.
    - history_download: List of reactions that have been downloaded.
    - now_download: List of reactions currently being downloaded.
    - log: Log of upload and download operations.
    """
...
class RoboticArm:
    """
    A class to represent a robotic arm for transporting reactions.

    Attributes:
    - name: Name of the robotic arm.
    - template: Template for the robotic arm.
    - ip: IP address of the robotic arm.
    - local_path: Local path to the configuration file.
    - remote_path: Remote path to the configuration file.
    - status: Current status of the robotic arm.
    - waiting_transport: List of reactions waiting to be transported.
    - history_transport: List of reactions that have been transported.
    - now_transport: List of reactions currently being transported.
    - waiting_back_transport: List of reactions waiting to be back transported.
    - now_back_transport: List of reactions currently being back transported.
    - history_back_transport: List of reactions that have been back transported.
    - log: Log of transport operations.
    """
...
class UnchainedLab:
    """
    A class to represent the Unchained Labs automation system.

    Attributes:
    - name: Name of the Unchained Lab system.
    - local_path: Local path to the configuration file.
    - remote_path: Remote path to the configuration file.
    - template: Template for the Unchained Lab system.
    - re_parameter: Function for re-parameterizing the template.
    - ip: IP address of the Unchained Lab system.
    - log_path: Path to the log file.
    - status: Current status of the Unchained Lab system.
    - waiting_prepare: List of reactions waiting to be prepared.
    - history_prepare: List of reactions that have been prepared.
    - now_prepare: List of reactions currently being prepared.
    - waiting_sample: List of reactions waiting to be sampled.
    - history_sample: List of reactions that have been sampled.
    - now_sample: List of reactions currently being sampled.
    - waiting_transfer: List of reactions waiting to be transferred.
    - history_transfer: List of reactions that have been transferred.
    - now_transfer: List of reactions currently being transferred.
    - waiting_back_transfer: List of reactions waiting to be back transferred.
    - history_back_transfer: List of reactions that have been back transferred.
    - now_back_transfer: List of reactions currently being back transferred.
...
class Plate:
    """
    A class to represent a reaction plate.

    Attributes:
    - plate_number: Unique identifier for the plate.
    - row: Number of rows on the plate.
    - column: Number of columns on the plate.
    - n_used: Number of wells used on the plate.
    - status: Current status of the plate ('reacting', 'idle', 'assigned', 'exhausted').
    - waiting: List of reactions waiting to be processed.
    - history: List of reactions that have been processed.
    - now: List of reactions currently being processed.
    - flow: Flow of operations for the plate.
    """
...
class Timer:
    """
    A class to represent a timer for tracking optimization time.

    Attributes:
    - opt_time: The total optimization time.
    - last_time: The last recorded time.
    """
    def __init__(self):
        self.opt_time = 0
...
class Data:
    """
    A class to represent the data for the optimization process.

    Attributes:
    - train_x: Training features tensor.
    - train_y: Training targets tensor.
    - all_x: All features tensor.
    - num: Number of data points.
    - current_optima: The current optimal value.
    """
    ...
    def update_data(self, reaction, space, max_time, n_break):
        """
        Updates the data with new reaction results.

        Parameters:
        - reaction: Dictionary of reactions.
        - space: The search space for the optimization.
        - max_time: Maximum allowed reaction time.
        - n_break: The current number of breaks in the optimization process.

        Returns:
        - n_break: Updated number of breaks in the optimization process.
        """
...
class Results:
    """
    A class to represent the results storage and update mechanism.

    Attributes:
    - store_path: Path to the results storage directory.
    """
    ...
    def update(self):
        """
        Updates the results by checking the storage directory for new files.

        Returns:
        - new_file_list: List of new files found in the storage directory.
        """
