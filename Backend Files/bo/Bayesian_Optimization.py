...
class Bo:
    """
    A class to represent Bayesian Optimization.

    Attributes:
    - space: The search space for the optimization.
    - train_x: Training data features.
    - train_y: Training data targets.
    - n_goal: Number of goals for multi-objective optimization.
    - ref_point: Reference point for hypervolume calculation.
    - x_dim: Dimensionality of the input space.
    - n_real: Number of real-valued dimensions.
    - n_cat: Number of categorical dimensions.
    - fixed_features_list: List of fixed features for categorical dimensions.
    - sampler: Sampler for QMC sampling.
    - bounds: Bounds for the optimization.
    - partitioning: Partitioning object for non-dominated hypervolume computation.
    """

    def __init__(self, space, train_x, train_y, n_goal, ref_point, num_samples: int = 2048,
                 seed: Optional[int] = None):
        """
        Initializes the Bo instance with provided parameters.
        ...
        - n_goal: Number of goals for multi-objective optimization.
        - ref_point: Reference point for hypervolume calculation.
        - num_samples (int): Number of samples for QMC sampling. Default is 2048.
        ...
        if self.ref_point:
            self.partitioning = NondominatedPartitioning(ref_point=self.ref_point, Y=self.train_y)
    ...
    def pi(self):
        ...
        Returns:
        - PI: The PI acquisition function.
        """
        if self.n_goal == 1:
            ...
            PI = ProbabilityHypervolumeImprovement(model=self.gp,
                                                   ...
                                                   sampler=SobolQMCNormalSampler(2 ** 14))
        return PI
    ...
    def ei(self):
        ...
        Returns:
        - EI: The EI acquisition function.
        """
        if self.n_goal == 1:
    ...
    def hv(self):
        """
        Computes the hypervolume of the current Pareto front.

        Returns:
        - hypervolume (float): The hypervolume.
        """
        return self.partitioning.compute_hypervolume()

    def pareto(self):
        """
        Retrieves the Pareto front of the current solutions.

        Returns:
        - pareto_Y (Tensor): The Pareto front.
        """
        return self.partitioning.pareto_Y

    def get_next_exps(self, q):
        """
        Gets the next set of experiments to perform.

        Parameters:
        - q (int): Number of experiments to propose.

        Returns:
        - candidate_sort (Tensor): Proposed candidate points.
        - qpi (Tensor): PI values for the proposed candidate points.
        """
        PI = self.pi()
        if self.n_goal == 1:
